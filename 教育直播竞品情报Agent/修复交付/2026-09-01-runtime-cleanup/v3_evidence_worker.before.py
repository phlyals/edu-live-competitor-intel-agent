...
from v3_projection_reconciler import reconcile_once as reconcile_projections
from v3_business_chain_worker import once as run_business_chain
        ...
        for row in rows:
            ...
            if row["object_type"] == "analysis":
                analysis_row = conn.execute(
                    "SELECT lineage_state,metadata_json FROM analyses WHERE analysis_id=?",
                    (row["object_id"],),
                ).fetchone()
                transition_id = ""
                if analysis_row:
                    transition_id = str(
                        json_object(analysis_row["metadata_json"]).get(
                            "recompute_request_id"
                        ) or ""
                    )
                enqueue_verified_analysis_projection_conn(
                    conn,
                    analysis_id=row["object_id"],
                    action="CURRENT",
            ...
            verified += 1
        conn.commit()
    reconciliation = reconcile_projections(connect_fn=connect)
    business_chain = run_business_chain(connect_fn=connect, init_db_fn=init_db)
    healthy = bool(
        blocked == 0
        and reconciliation.get("blocked_outbox", 0) == 0
        and reconciliation.get("missing_receipts", 0) == 0
        and reconciliation.get("receipt_mismatches", 0) == 0
        and business_chain.get("status") == "READY"
    )
    result = {"verified": verified, "blocked": blocked, "versioned_projections": projected, "reconciliation": reconciliation, "business_chain": business_chain, "checked_at": utc_now()}
    upsert_heartbeat("evidence-v3", "READY" if healthy else "DEGRADED", result, success=healthy)
    return result
