...
SCHEMA_PATH = V3_ROOT / "v3_schema.sql"
...
def enqueue_outbox_conn(
    ...
    *,
    ...
    conn.execute(
        ...
        "next_attempt_at,payload_hash,payload_json) VALUES(?,?,?,?,?,'PENDING',0,?,?,?,?) "
        "ON CONFLICT(dedupe_key) DO NOTHING",
        (outbox_id, key, object_type, object_id, destination, max_attempts, utc_now(), payload_hash, json_text(payload)),
    )
...
def enqueue_outbox(*, object_type: str, object_id: str, destination: str, payload: dict, max_attempts: int = OUTBOX_MAX_ATTEMPTS) -> str:
    with connect() as conn:
...
def complete_outbox(outbox_id: str, receipt: dict) -> None:
    with connect() as conn:
        ...
        conn.execute(
            "INSERT INTO delivery_receipts(receipt_id,outbox_id,destination,object_type,object_id,status,payload_hash,sent_at,verified_at,receipt_json) "
