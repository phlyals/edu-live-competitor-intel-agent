...
CANDIDATE = ROOT / "candidate" / "runtime" / "v3"
...
worker = load("candidate_v3_worker", CANDIDATE / "v3_worker.py")
pipeline = load("candidate_v3_pipeline", CANDIDATE / "v3_pipeline_worker.py")
...
class CandidateCodeTests(unittest.TestCase):
    ...
    def test_fresh_schema_contains_fail_closed_lifecycle_columns(self):
        ...
        with self.connect(db) as conn:
            conn.executescript((CANDIDATE / "v3_schema.sql").read_text(encoding="utf-8"))
