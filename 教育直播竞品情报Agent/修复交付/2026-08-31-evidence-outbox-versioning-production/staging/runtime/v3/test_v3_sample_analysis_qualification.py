...
class PipelineGateTests(unittest.TestCase):
    def setUp(self):
        ...
        with connect_factory(self.db)() as conn:
            ...
            CREATE TABLE outbox(outbox_id TEXT PRIMARY KEY,dedupe_key TEXT UNIQUE,object_type TEXT,object_id TEXT,destination TEXT,status TEXT,attempts INTEGER,max_attempts INTEGER,next_attempt_at TEXT,payload_hash TEXT,payload_json TEXT,scope TEXT,qualification_status TEXT,projection_version TEXT,artifact_digest TEXT,evidence_bundle_id TEXT,evidence_manifest_hash TEXT,evidence_verified_at TEXT,projection_binding_status TEXT DEFAULT 'UNCLASSIFIED',UNIQUE(destination,object_type,object_id,projection_version));
            """)
    ...
    def test_old_spec_does_not_block_new_full_identity_and_same_spec_is_idempotent(self):
        ...
        old_artifact = self.root / "old.analysis.json"; old_artifact.write_text("{}")
        old_artifact_digest = contract.file_sha256(old_artifact)
        with connect_factory(self.db)() as conn:
            conn.execute("INSERT INTO live_sessions VALUES('s','MEDIA_COMPLETE','COMPLETE',?)", (json.dumps(session_meta()),))
            conn.execute("INSERT INTO recording_segments VALUES('seg-full','s','COMPLETE','CANONICAL_ACTIVE','media-checksum','media.ts')")
            ...
            conn.execute("INSERT INTO analyses(analysis_id,session_id,transcript_id,analysis_type,source_digest,status,output_path,lineage_state,scope,qualification_status,transcript_content_digest,analysis_spec_version,model_version,prompt_version,artifact_digest,metadata_json) VALUES('old','s','full','single_session',?,'COMPLETE',?,'CURRENT','FORMAL_SINGLE_SESSION','FULL_SESSION_QUALIFIED',?,'old-spec','old-model','old-prompt',?,'{}')",(content,str(old_artifact),content,old_artifact_digest))
            ...
            conn.execute("INSERT INTO evidence_bundles VALUES('bundle:old','analysis','old','VERIFIED',?,?, '2026-08-30T00:00:01Z','FORMAL_SINGLE_SESSION','FULL_SESSION_QUALIFIED','{}')",(str(old_artifact),old_artifact_digest))
        with patch.object(pipeline,"connect",connect_factory(self.db)),patch.object(pipeline,"load_pipeline_config",return_value={}):
            self.assertEqual(pipeline.create_analysis_tasks(),1)
            self.assertEqual(pipeline.create_analysis_tasks(),0)
        with connect_factory(self.db)() as conn:
...
class AnalysisConsumerGateTests(unittest.TestCase):
    ...
    def test_exact_qualified_full_transcript_can_complete(self):
        ...
        with patch.object(analysis, "connect", connect_factory(self.db)), patch.object(analysis, "init_db"), patch.object(analysis, "upsert_heartbeat", heartbeat), patch.object(analysis, "ANALYSIS_ROOT", analysis_root), patch.object(analysis, "request_chunk", side_effect=strict_response):
            result = analysis.once()
...
class EvidenceAndProjectionTests(unittest.TestCase):
    ...
    def test_evidence_worker_invalidates_sample_bundle_without_deleting_artifact(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(evidence,"connect",connect_factory(db)),patch.object(evidence,"init_db"),patch.object(evidence,"upsert_heartbeat"),patch.object(evidence,"reconcile_projections",return_value={}):
    ...
    def formal_evidence_case(self, directory, *, analysis_digest=None, bundle_digest=None, lineage_digest=None):
        ...
        with patch.object(evidence,"connect",connect_factory(db)),patch.object(evidence,"init_db"),patch.object(evidence,"upsert_heartbeat"),patch.object(evidence,"enqueue_verified_analysis_projection_conn"),patch.object(evidence,"reconcile_projections",return_value={}):
            result=evidence.once()
