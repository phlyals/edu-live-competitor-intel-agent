...
class CoverageVersionIntegrationTests(RuntimeCase):
    ...
    def test_new_source_binding_version_reruns_model_and_supersedes_old(self):
        ...
        self.assertEqual({row["object_id"] for row in outbox}, {"analysis-old"})
        self.assertEqual({row["status"] for row in outbox}, {"PENDING"})
        ...
        self.assertEqual(evidence_result["versioned_projections"], 1)
        with self.connect() as conn:
