...
def migration_health(cur) -> dict:
    ...
    if not missing:
        ...
        cur.execute("SELECT analysis_id,transcript_id,transcript_content_digest FROM analyses WHERE scope=%s AND qualification_status=%s AND lineage_state IN ('CURRENT','CANDIDATE')", (ANALYSIS_SCOPE_FORMAL, QUALIFIED))
        lineage_errors = []
