...
def enqueue_outbox_conn(
    ...
    *,
    ...
    projection_binding_status: str = "UNCLASSIFIED",
) -> str:
    if max_attempts <= 0:
    ...
    if projection_version is not None:
        if not all((artifact_digest, evidence_bundle_id, evidence_manifest_hash, evidence_verified_at)):
            raise ValueError("versioned projection requires verified evidence identity")
        if projection_binding_status != "VERSIONED_EVIDENCE":
            raise ValueError("versioned projection must use VERSIONED_EVIDENCE binding")
        key = f"{destination}:{object_type}:{object_id}:{projection_version}:{payload_hash}"
        conn.execute(
            "INSERT INTO outbox(outbox_id,dedupe_key,object_type,object_id,destination,status,attempts,max_attempts,"
            "next_attempt_at,payload_hash,payload_json,scope,qualification_status,projection_version,"
            "artifact_digest,evidence_bundle_id,evidence_manifest_hash,evidence_verified_at,projection_binding_status) "
            "VALUES(?,?,?,?,?,'PENDING',0,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT DO NOTHING",
            (
                outbox_id, key, object_type, object_id, destination, max_attempts,
                utc_now(), payload_hash, json_text(payload), scope,
                qualification_status, projection_version, artifact_digest,
                evidence_bundle_id, evidence_manifest_hash, evidence_verified_at,
                projection_binding_status,
            ),
        )
        row = conn.execute(
            "SELECT outbox_id,payload_hash,dedupe_key FROM outbox WHERE dedupe_key=? "
            "OR (destination=? AND object_type=? AND object_id=? AND projection_version=?) "
            "ORDER BY CASE WHEN dedupe_key=? THEN 0 ELSE 1 END LIMIT 1",
            (key, destination, object_type, object_id, projection_version, key),
        ).fetchone()
        if row and (row["payload_hash"] != payload_hash or row["dedupe_key"] != key):
            raise RuntimeError("projection version already exists with different payload")
    else:
...
def enqueue_outbox(*, object_type: str, object_id: str, destination: str, payload: dict,
                   ...
                   evidence_manifest_hash: str | None = None,
                   evidence_verified_at: str | None = None,
                   projection_binding_status: str = "UNCLASSIFIED") -> str:
    with connect() as conn:
        return enqueue_outbox_conn(
            ...
            projection_binding_status=projection_binding_status,
        )
...
def complete_outbox(outbox_id: str, receipt: dict) -> None:
    with connect() as conn:
        ...
        if "projection_version" in row.keys():
            conn.execute(
                "INSERT INTO delivery_receipts(receipt_id,outbox_id,destination,object_type,object_id,status,payload_hash,sent_at,verified_at,receipt_json,scope,qualification_status,projection_version,artifact_digest,evidence_bundle_id,evidence_manifest_hash,evidence_verified_at,projection_binding_status) "
                ...
                (
                    ...
                    row["projection_version"], row["artifact_digest"],
                    row["evidence_bundle_id"], row["evidence_manifest_hash"],
                    row["evidence_verified_at"], row["projection_binding_status"],
                ),
            )
        else:
            conn.execute(
...
def status_snapshot() -> dict:
    with connect() as conn:
        tables = ["inbox_messages", "tasks", "task_attempts", "checkpoints", "domain_events", "products", "competitors", "identities", "scan_runs", "scan_observations", "product_competitors", "monitor_targets", "live_sessions", "recording_jobs", "recording_segments", "transcripts", "analyses", "comparisons", "comparison_evidence", "strategy_versions", "version_observations", "strategy_candidates", "strategy_evidence", "knowledge_versions", "knowledge_diffs", "knowledge_publish_receipts", "review_items", "retention_jobs", "approvals", "outbox", "delivery_receipts", "dead_letters", "lineage_edges", "recompute_requests", "evidence_bundles", "heartbeats", "task_leases", "identity_evidence", "identity_conflicts", "worker_nodes", "recording_leases", "recording_gaps", "media_manifests", "deployment_releases", "projection_reconciliations", "capacity_test_runs", "fault_drill_runs", "audit_log"]
