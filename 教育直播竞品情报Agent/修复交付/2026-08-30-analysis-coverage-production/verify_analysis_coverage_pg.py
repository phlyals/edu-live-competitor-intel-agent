#!/usr/bin/env python3
"""Run the coverage-version upgrade against a disposable PostgreSQL clone."""
...
def main() -> int:
    ...
    clone = "edu_analysis_coverage_test_" + uuid.uuid4().hex[:10]
        ...
        try:
            ...
            with patch.object(analysis, "connect", side_effect=connect), \
                    ...
                    patch.object(
                        ...
                        side_effect=AssertionError("model must not run for exact legacy revalidation"),
                    ):
            ...
            current = [
                ...
                and row["analysis_spec_version"] == "single-session-evidence-v3-coverage"
            ]
            old = [
                row for row in analyses
                if row["analysis_spec_version"] == "single-session-evidence-v2"
            ]
