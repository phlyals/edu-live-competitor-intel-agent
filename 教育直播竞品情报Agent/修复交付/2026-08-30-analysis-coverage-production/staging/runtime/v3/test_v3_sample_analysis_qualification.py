...
class PipelineGateTests(unittest.TestCase):
    def setUp(self):
        ...
        with connect_factory(self.db)() as conn:
            ...
            CREATE TABLE transcripts(transcript_id TEXT PRIMARY KEY,session_id TEXT,source_digest TEXT,status TEXT,output_path TEXT,scope TEXT,qualification_status TEXT,metadata_json TEXT);
            CREATE TABLE analyses(analysis_id TEXT PRIMARY KEY,session_id TEXT,transcript_id TEXT,analysis_type TEXT,source_digest TEXT,status TEXT,output_path TEXT,lineage_state TEXT,scope TEXT,qualification_status TEXT,transcript_content_digest TEXT,analysis_spec_version TEXT,model_version TEXT,prompt_version TEXT,artifact_digest TEXT,metadata_json TEXT,UNIQUE(transcript_id,analysis_type,transcript_content_digest,analysis_spec_version,model_version,prompt_version));
            CREATE TABLE lineage_edges(edge_id TEXT PRIMARY KEY,downstream_type TEXT,downstream_id TEXT,upstream_type TEXT,upstream_id TEXT,upstream_version TEXT,state TEXT DEFAULT 'CURRENT',created_at TEXT,UNIQUE(downstream_type,downstream_id,upstream_type,upstream_id,upstream_version));
            """)
    ...
    def test_sample_is_ignored_and_invalidated_history_does_not_block_full(self):
        ...
        with connect_factory(self.db)() as conn:
            ...
            conn.execute("INSERT INTO transcripts VALUES('sample','s','sample-digest','COMPLETE',?,'SAMPLE','SAMPLE_NONQUALIFYING',?)", (str(sample), json.dumps({"coverage_scope":"SAMPLE","sample_only":True,"sample_seconds":300,"source_segment_id":"seg-sample"})))
            conn.execute("INSERT INTO transcripts VALUES('full','s',?,'COMPLETE',?,'FULL_SESSION','FULL_SESSION_QUALIFIED',?)", (canonical_source_digest(), str(full), json.dumps(full_meta())))
    ...
    def test_noncanonical_full_transcript_is_ignored(self):
        ...
        with connect_factory(self.db)() as conn:
            ...
            conn.execute("INSERT INTO transcripts VALUES('full','s',?,'COMPLETE',?,'FULL_SESSION','FULL_SESSION_QUALIFIED',?)", (canonical_source_digest(), str(artifact), json.dumps(full_meta("stale"))))
    ...
    def test_same_canonical_segment_with_stale_source_digest_is_ignored(self):
        ...
        with connect_factory(self.db)() as conn:
            conn.execute("INSERT INTO live_sessions VALUES('s','MEDIA_COMPLETE','COMPLETE',?)", (json.dumps(session_meta()),))
            conn.execute("INSERT INTO recording_segments VALUES('seg-full','s','COMPLETE','CANONICAL_ACTIVE','media-checksum','media.ts')")
            conn.execute("INSERT INTO transcripts VALUES('stale','s','old-source-digest','COMPLETE',?,'FULL_SESSION','FULL_SESSION_QUALIFIED',?)", (str(artifact), json.dumps(full_meta())))
        with patch.object(pipeline, "connect", connect_factory(self.db)), patch.object(pipeline, "load_pipeline_config", return_value={}):
            self.assertEqual(pipeline.create_analysis_tasks(), 0)
    ...
    def test_old_spec_does_not_block_new_full_identity_and_same_spec_is_idempotent(self):
        ...
        with connect_factory(self.db)() as conn:
            conn.execute("INSERT INTO live_sessions VALUES('s','MEDIA_COMPLETE','COMPLETE',?)", (json.dumps(session_meta()),))
            conn.execute("INSERT INTO recording_segments VALUES('seg-full','s','COMPLETE','CANONICAL_ACTIVE','media-checksum','media.ts')")
            conn.execute("INSERT INTO transcripts VALUES('full','s',?,'COMPLETE',?,'FULL_SESSION','FULL_SESSION_QUALIFIED',?)", (canonical_source_digest(),str(artifact),json.dumps(full_meta())))
        ...
        with patch.object(pipeline,"connect",connect_factory(self.db)),patch.object(pipeline,"load_pipeline_config",return_value={}):
            ...
            self.assertEqual(pipeline.create_analysis_tasks(),0)
        with connect_factory(self.db)() as conn:
            rows=conn.execute("SELECT transcript_id,transcript_content_digest,analysis_spec_version,model_version,prompt_version FROM analyses ORDER BY analysis_id").fetchall()
        self.assertEqual(len(rows),2)
        self.assertTrue(any(row["analysis_spec_version"]==contract.ANALYSIS_SPEC_VERSION and row["model_version"]==contract.MODEL_VERSION and row["prompt_version"]==contract.PROMPT_VERSION for row in rows))
...
class AnalysisConsumerGateTests(unittest.TestCase):
    def setUp(self):
        ...
        with connect_factory(self.db)() as conn:
            ...
            CREATE TABLE lineage_edges(edge_id TEXT PRIMARY KEY,downstream_type TEXT,downstream_id TEXT,upstream_type TEXT,upstream_id TEXT,upstream_version TEXT,state TEXT,created_at TEXT);
    ...
    def test_manual_sample_analysis_is_invalidated_before_model_call(self):
        ...
        with connect_factory(self.db)() as conn:
            ...
            conn.execute("INSERT INTO transcripts VALUES('sample','s','sample-digest','COMPLETE',?,'SAMPLE','SAMPLE_NONQUALIFYING',?)", (str(artifact), json.dumps({"coverage_scope":"SAMPLE","sample_only":True,"source_segment_id":"seg"})))
            ...
            conn.execute("INSERT INTO lineage_edges VALUES('e','analysis','a','transcript','sample',?,'CURRENT','now')",(content_digest,))
    ...
    def test_lineage_mismatch_is_blocked_before_model_call(self):
        ...
        with connect_factory(self.db)() as conn:
            ...
            conn.execute("INSERT INTO lineage_edges VALUES('e','analysis','a','transcript','other',?,'CURRENT','now')",(content_digest,))
        result, request, _ = self.run_once()
    ...
    def test_exact_qualified_full_transcript_can_complete(self):
        ...
        with connect_factory(self.db)() as conn:
            ...
            conn.execute("INSERT INTO transcripts VALUES('full','s',?,'COMPLETE',?,'FULL_SESSION','FULL_SESSION_QUALIFIED',?)", (canonical_source_digest(), str(transcript), json.dumps(full_meta())))
            ...
            conn.execute("INSERT INTO lineage_edges VALUES('e','analysis','a','transcript','full',?,'CURRENT','now')",(content_digest,))
        heartbeat = Mock(); enqueue = Mock()
        accepted_coverage = {
            "schema_version": 2,
            "coverage_scope": "FULL_SESSION",
            "analysis_coverage_rate": 1.0,
            "source_transcript_coverage_rate": 1.0,
            "segment_coverage_rate": 1.0,
            "timestamps_valid": True,
            "source_segment_ids_valid": True,
        ...
        with patch.object(analysis, "connect", connect_factory(self.db)), patch.object(analysis, "init_db"), patch.object(analysis, "upsert_heartbeat", heartbeat), patch.object(analysis, "ANALYSIS_ROOT", analysis_root), patch.object(analysis, "request_analysis", return_value=({"schema_version":"test","modules":[],"analysis_coverage":accepted_coverage},{"provider":"test"})), patch.object(analysis, "enqueue_outbox_conn", enqueue):
            result = analysis.once()
    ...
    def test_same_segment_id_with_stale_source_digest_is_blocked(self):
        ...
        with connect_factory(self.db)() as conn:
            conn.execute("INSERT INTO live_sessions VALUES('s','MEDIA_COMPLETE','COMPLETE',?)",(json.dumps(session_meta()),))
            conn.execute("INSERT INTO recording_segments VALUES('seg-full','s','COMPLETE','CANONICAL_ACTIVE','media-checksum','media.ts')")
            conn.execute("INSERT INTO transcripts VALUES('full','s','old-source-digest','COMPLETE',?,'FULL_SESSION','FULL_SESSION_QUALIFIED',?)",(str(artifact),json.dumps(full_meta())))
...
class EvidenceAndProjectionTests(unittest.TestCase):
    ...
    def formal_evidence_case(self, directory, *, analysis_digest=None, bundle_digest=None, lineage_digest=None):
        ...
        with connect_factory(db)() as conn:
            conn.executescript("""
            CREATE TABLE analyses(analysis_id TEXT PRIMARY KEY,session_id TEXT,transcript_id TEXT,analysis_type TEXT,status TEXT,output_path TEXT,lineage_state TEXT,scope TEXT,qualification_status TEXT,transcript_content_digest TEXT,analysis_spec_version TEXT,model_version TEXT,prompt_version TEXT,artifact_digest TEXT,metadata_json TEXT);
            CREATE TABLE transcripts(transcript_id TEXT PRIMARY KEY,output_path TEXT,scope TEXT,qualification_status TEXT);
            CREATE TABLE lineage_edges(edge_id TEXT PRIMARY KEY,downstream_type TEXT,downstream_id TEXT,upstream_type TEXT,upstream_id TEXT,upstream_version TEXT,state TEXT);
            CREATE TABLE evidence_bundles(bundle_id TEXT PRIMARY KEY,object_type TEXT,object_id TEXT,status TEXT,manifest_path TEXT,manifest_hash TEXT,verified_at TEXT,scope TEXT,qualification_status TEXT,metadata_json TEXT);
            """)
            ...
            conn.execute("INSERT INTO analyses VALUES('a','s','t','single_session','COMPLETE',?,'CURRENT','FORMAL_SINGLE_SESSION','FULL_SESSION_QUALIFIED',?,?,?,?,?,?)",(str(analysis_path),transcript_digest,contract.ANALYSIS_SPEC_VERSION,contract.MODEL_VERSION,contract.PROMPT_VERSION,analysis_digest or actual,json.dumps(meta)))
            conn.execute("INSERT INTO transcripts VALUES('t',?,'FULL_SESSION','FULL_SESSION_QUALIFIED')",(str(transcript),))
            conn.execute("INSERT INTO lineage_edges VALUES('e','analysis','a','transcript','t',?,'CURRENT')",(lineage_digest or transcript_digest,))
