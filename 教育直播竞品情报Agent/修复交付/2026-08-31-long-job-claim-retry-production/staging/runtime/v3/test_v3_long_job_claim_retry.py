...
class AnalysisProcessIntegrationTests(unittest.TestCase):
    def setUp(self):
        ...
        conn.executescript(SCHEMA + """
            ...
            CREATE TABLE lineage_edges(
              ...
              upstream_version TEXT,state TEXT
            );
        ...
        conn.execute(
            "INSERT INTO lineage_edges VALUES('e','analysis','a','transcript','t',?,'CURRENT')",
            (transcript_digest,),
        )
    ...
    def test_complete_publication_is_fenced_versioned_and_atomic(self):
        ...
        self.assertEqual(conn.execute("SELECT status FROM outbox").fetchone()[0], "HELD_EVIDENCE")
