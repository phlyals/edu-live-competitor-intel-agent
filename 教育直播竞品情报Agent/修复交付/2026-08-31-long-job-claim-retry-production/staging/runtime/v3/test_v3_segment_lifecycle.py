    ...
    def test_pipeline_only_attempts_canonical_active(self):
        ...
        with self.connect(db) as conn:
            conn.executescript("""
                ...
                CREATE TABLE transcripts(transcript_id TEXT PRIMARY KEY,session_id TEXT,source_digest TEXT,engine TEXT,model TEXT,status TEXT,language TEXT,source_path TEXT,output_path TEXT,low_confidence_count INTEGER DEFAULT 0,created_at TEXT,updated_at TEXT,attempts INTEGER NOT NULL DEFAULT 0,max_attempts INTEGER NOT NULL DEFAULT 3,next_attempt_at TEXT,last_attempt_at TEXT,lease_owner TEXT,lease_until TEXT,lease_epoch INTEGER NOT NULL DEFAULT 0,last_error_type TEXT,last_error TEXT,checkpoint_json TEXT NOT NULL DEFAULT '{}',scope TEXT NOT NULL DEFAULT 'UNCLASSIFIED',qualification_status TEXT NOT NULL DEFAULT 'UNCLASSIFIED',metadata_json TEXT,UNIQUE(session_id,source_digest));
            """)
