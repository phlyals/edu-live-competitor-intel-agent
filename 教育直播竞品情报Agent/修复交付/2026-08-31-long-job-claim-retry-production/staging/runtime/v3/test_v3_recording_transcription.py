...
class FullTranscriptTests(RecordingCase):
    ...
    def test_short_asr_result_is_not_complete(self):
        ...
        with self.connect() as c:
            row=c.execute("SELECT status,attempts,lease_owner,next_attempt_at,last_error_type,checkpoint_json FROM transcripts WHERE transcript_id!='sample'").fetchone()
        self.assertEqual(row['status'],'RETRY_WAIT')
        self.assertEqual(row['attempts'],1)
        self.assertIsNone(row['lease_owner'])
        self.assertIsNotNone(row['next_attempt_at'])
        self.assertEqual(row['last_error_type'],'ASR_DURATION_MISMATCH')
        checkpoint=json.loads(row['checkpoint_json'])
        self.assertEqual(checkpoint['phase'],'AUDIO_READY')
        self.assertEqual(checkpoint['audio_duration_seconds'],600)
