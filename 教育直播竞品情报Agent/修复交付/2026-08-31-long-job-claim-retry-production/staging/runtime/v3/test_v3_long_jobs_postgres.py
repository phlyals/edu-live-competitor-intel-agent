...
from v3_long_jobs import ANALYSIS, TRANSCRIPT, LeaseLostError, claim_next, reconcile_exhausted, save_checkpoint
...
class RealPostgresConcurrencyTests(unittest.TestCase):
        ...
        with connect(DSN) as conn:
            ...
            conn.execute("""
                CREATE TABLE transcripts(
                  transcript_id TEXT PRIMARY KEY, session_id TEXT NOT NULL,
                  source_digest TEXT NOT NULL, engine TEXT NOT NULL, model TEXT NOT NULL,
                  ...
                  next_attempt_at TEXT, last_attempt_at TEXT, lease_owner TEXT,
                  lease_until TEXT, lease_epoch BIGINT NOT NULL DEFAULT 0,
                  last_error_type TEXT, last_error TEXT,
    ...
    def setUp(self):
        with connect(DSN) as conn:
            ...
            conn.execute("TRUNCATE transcripts")
            conn.commit()
    ...
    def insert_transcript(self):
        with connect(DSN) as conn:
            conn.execute(
                "INSERT INTO transcripts(transcript_id,session_id,source_digest,engine,model,status,next_attempt_at) "
                "VALUES('t','s','d','faster-whisper','small','PENDING','2026-08-30T00:00:00.000Z')"
            )
            conn.commit()

    def test_transcript_simultaneous_claim_has_one_winner(self):
        ...
        def worker(name):
            ...
            with connect(DSN) as conn:
                row = claim_next(
                    conn, TRANSCRIPT, name, now="2026-08-30T00:00:01.000Z"
                )
            return row["transcript_id"] if row else None

        with ThreadPoolExecutor(max_workers=2) as pool:
            results = list(pool.map(worker, ("tw1", "tw2")))
        self.assertEqual(sorted(value for value in results if value), ["t"])

    def test_transcript_reclaim_fences_old_checkpoint(self):
        self.insert_transcript()
        with connect(DSN) as conn:
            old = claim_next(
                conn, TRANSCRIPT, "old", now="2026-08-30T00:00:01.000Z",
                lease_seconds=1,
            )
        with connect(DSN) as conn:
            new = claim_next(
                conn, TRANSCRIPT, "new", now="2026-08-30T00:00:03.000Z"
            )
        with connect(DSN) as conn, self.assertRaises(LeaseLostError):
            save_checkpoint(conn, TRANSCRIPT, old, {"phase": "STALE"})
        with connect(DSN) as conn:
            save_checkpoint(
                conn, TRANSCRIPT, new, {"phase": "CURRENT"},
                now="2026-08-30T00:00:04.000Z",
            )
        with connect(DSN) as conn:
            row = conn.execute(
                "SELECT lease_epoch,checkpoint_json FROM transcripts WHERE transcript_id='t'"
            ).fetchone()
    ...
    def test_expired_lease_reclaim_fences_old_epoch(self):
        ...
        with connect(DSN) as conn:
            save_checkpoint(
                conn, ANALYSIS, new, {"phase": "CURRENT"},
    ...
    def test_expired_owner_is_fenced_before_any_reclaim(self):
        self.insert(1)
        with connect(DSN) as conn:
            expired = claim_next(
        ...
        with connect(DSN) as conn, self.assertRaises(LeaseLostError):
            save_checkpoint(
                conn, ANALYSIS, expired, {"phase": "STALE"},
                now="2026-08-30T00:00:03.000Z",
            )
        with connect(DSN) as conn:
            row = conn.execute(
                "SELECT status,lease_owner,lease_epoch,checkpoint_json "
                "FROM analyses WHERE analysis_id='a000'"
            ).fetchone()
        self.assertEqual(
            (row["status"], row["lease_owner"], row["lease_epoch"], row["checkpoint_json"]),
            ("RUNNING", "expired", 1, "{}"),
        )
