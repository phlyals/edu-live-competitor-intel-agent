...
SPECS = {
    ...
    ANALYSIS: JobSpec(
        ...
        due_statuses=("PENDING", "WAITING_MODEL", "RETRY_WAIT"),
    ),
}
...
def _fenced_update(
    conn: Any, kind: str, job: dict, assignments: dict[str, Any], *, commit_transaction: bool = True
) -> None:
    spec = _spec(kind)
    allowed = {
    ...
    params.extend([job[spec.id_column], job["lease_owner"], int(job["lease_epoch"])])
    identity_sql = ""
    if kind == ANALYSIS:
        for column in (
    ...
    cursor = conn.execute(
        ...
        f"AND status='RUNNING' AND lease_owner=? AND lease_epoch=?{identity_sql}",
        params,
    )
...
def renew(conn: Any, kind: str, job: dict, *, lease_seconds: int = 600, now: str | None = None) -> str:
    now = now or utc_now()
    lease_until = after_seconds(lease_seconds, now=now)
    _fenced_update(conn, kind, job, {"lease_until": lease_until, "updated_at": now})
...
def save_checkpoint(conn: Any, kind: str, job: dict, checkpoint: dict, *, now: str | None = None) -> None:
    ...
    _fenced_update(conn, kind, job, {"checkpoint_json": serialized, "updated_at": now})
...
def finish(
    ...
    *,
    ...
    _fenced_update(conn, kind, job, values, commit_transaction=commit_transaction)
...
def fail_or_retry(
    ...
    *,
    ...
    _fenced_update(conn, kind, job, assignments)
    return status
