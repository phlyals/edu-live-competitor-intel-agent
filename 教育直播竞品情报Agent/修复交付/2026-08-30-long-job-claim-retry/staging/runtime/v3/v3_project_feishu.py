...
def project(*, dry_run: bool = False, product_id: str | None = None) -> dict:
    ...
    if product_id:
        ...
        sessions, transcripts, analyses, strategies, versions = [], [], [], [], []
    ...
    for analysis in analyses:
        fields = {"分析ID": analysis["analysis_id"], "当前场次ID": analysis["session_id"], "分析类型": "单场分析", "状态": "完成" if analysis["status"] == "COMPLETE" else "分析中", "分析文档": analysis.get("output_path") or "", "证据说明": "Runtime V3 lineage_state=" + str(analysis.get("lineage_state") or "CURRENT"), "变化摘要": "", "整体相似度": 0}
        reports.append(upsert(config, tables["07_场次分析"], "分析ID", analysis["analysis_id"], fields, dry_run=dry_run))
    for strategy in strategies:
        fields = {"候选ID": strategy["candidate_id"], "策略候选": strategy["strategy_type"], "验证状态": "单场观察", "正式写入状态": "未申请", "类别": "其他", "来源场次ID": strategy.get("session_id") or "", "来源同行ID": "", "证据时间戳": "", "人工结论": "待审核", "正式知识Diff": ""}
...
def project_analysis(analysis_id: str, *, dry_run: bool = False) -> dict:
    ...
    fields = {"分析ID": row["analysis_id"], "当前场次ID": row["session_id"], "分析类型": "单场分析", "状态": "完成" if row["status"] == "COMPLETE" else "分析中", "分析文档": row["output_path"] or "", "证据说明": "Runtime V3 lineage_state=" + str(row["lineage_state"] or "CURRENT"), "变化摘要": "", "整体相似度": 0}
